...
public class CouponsAdapter extends RecyclerView.Adapter<CouponsAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        ...
        if (currency.isEmpty())
           ...
           // holder.coupon_discount.setText(new DecimalFormat("#0.00").format(Double.parseDouble(coupon.getDiscount())) + " " + currency);
            holder.coupon_discount.setText(coupon.getDiscount()+ " " + currency);

        if (coupon.getDiscountType().equalsIgnoreCase("fixed_cart") || coupon.getDiscountType().equalsIgnoreCase("percent")) {
            holder.coupon_type.setText(coupon.getType().toString());
