...
import com.mytodokart.app.adapters.CartItemsAdapter;
...
import com.mytodokart.app.models.Plans;
...
public class My_Cart extends Fragment {
    ...
    public LinearLayout cart_bottom_bar;
    public RelativeLayout cart_mp_wraning, cart_total_wrap;
    ...
    Call<SubscriptionData> networkCall;
    List<Plans> subscriptionDataList = new ArrayList<>();
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        finalCartItemsList = user_cart_db.getCartItems();
        int nn = 0;
        for (nn = 0; nn < finalCartItemsList.size(); nn++) {
            if (finalCartItemsList.get(nn).getCustomersBasketProduct().getProducts_subscription().equalsIgnoreCase("1")) {
                if (prefsManager.getProductSubscription().equalsIgnoreCase("")) {
                    User_Cart_DB user_cart_db = new User_Cart_DB();
                    user_cart_db.deleteCartItem(finalCartItemsList.get(nn).getCustomersBasketId());
                }
            }
        }
        ...
        cart_bottom_bar = (LinearLayout) rootView.findViewById(R.id.cart_bottom_bar);
        cart_total_wrap = (RelativeLayout) rootView.findViewById(R.id.cart_total_wrap);
        cart_mp_wraning = (RelativeLayout) rootView.findViewById(R.id.cart_mp_wraning);
        ...
        //( (MainActivity) getActivity() ).getActionBar().setH;

        return rootView;
    }

    private void getSubscriptionPlans() {
        networkCall = APIClient.getInstance().getPlans();
        networkCall.enqueue(new Callback<SubscriptionData>() {
            @Override
            public void onResponse(Call<SubscriptionData> call, retrofit2.Response<SubscriptionData> response) {
                if (response.isSuccessful()) {
                    // Check the Success status
                    if (response.body().success.equalsIgnoreCase("1")) {
                        subscriptionDataList = response.body().plans_array;
                        for (int i = 0; i < subscriptionDataList.size(); i++) {
                            plans.add(subscriptionDataList.get(i).plan_label);
                        }
                        // Initialize the AddressListAdapter for RecyclerView
                        cartItemsAdapter = new CartItemsAdapter(getContext(), finalCartItemsList, My_Cart.this, plans);
    ...
    public static CartProduct GetCartProduct(int product_id) {
        User_Cart_DB user_cart_db = new User_Cart_DB();
        CartProduct cartProduct = user_cart_db.getCartProduct(product_id);
        return cartProduct;
    }
    ...
    public static boolean checkSubscribed(String type) {
        User_Cart_DB user_cart_db = new User_Cart_DB();
        List<CartProduct> itemsList = new ArrayList<>();
        itemsList = user_cart_db.getCartItems();
        int cc = itemsList.size();
        int i = 0;
        boolean subs = false;
        System.out.println("Cart Size " + cc);
        if (type.equalsIgnoreCase("1")) {
            for (i = 0; i < cc; i++) {
                String tt = itemsList.get(i).getCustomersBasketProduct().getProducts_subscription();
                System.out.println("CHECK  " + tt);
                if (itemsList.get(i).getCustomersBasketProduct().getProducts_subscription().equalsIgnoreCase("1")) {
            ...
            for (i = 0; i < cc; i++) {
                String tt = itemsList.get(i).getCustomersBasketProduct().getProducts_subscription();
                System.out.println("NOT CHECK  " + tt);
                if (tt.equalsIgnoreCase("0")) {
                    System.out.println("Not Subscribed " + i);
                    subs = true;
                }
            }
        }
    ...
    public static void UpdateCartSubscriptionStartEndDateItem(CartProduct cartProduct) {
        User_Cart_DB user_cart_db = new User_Cart_DB();
        user_cart_db.updateCartSubscriptionStartEndItem(cartProduct);
    }

    public static void UpdateCartItem(String qnt, String price, String pid) {
        User_Cart_DB user_cart_db = new User_Cart_DB();
        user_cart_db.updateCartItem(qnt, price, pid);
    }
    ...
    public static int getCartProductQnt(int cart_item_id) {
        User_Cart_DB user_cart_db = new User_Cart_DB();
        return user_cart_db.getCartProductQnt(cart_item_id);
    }
    ...
    private class MyTask extends AsyncTask<String, Void, String> {
        ...
        protected void onPostExecute(String s) {
            ...
            for (int i = 0; i < finalCartItemsList.size(); i++) {
                ...
                finalCartItemsList.get(i).getCustomersBasketProduct().setProducts_zip_codes(cartProducts.get(i).getProducts_zip_codes());
            }
            cartItemsAdapter.notifyDataSetChanged();
        }
    }
    ...
    public void RequestProductDetails(final int position, final int products_id) throws IOException {
        ...
        getAllProducts.setUser_zip(prefsManager.getUserZipcode());
        Call<ProductData> call = APIClient.getInstance().getAllProducts(getAllProducts);
    ...
    public class CheckStockTask extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            if (isAllStockValid(stocks)) {
                ...
                FragmentManager fragmentManager = getFragmentManager();
                fragmentManager.beginTransaction().add(R.id.main_fragment, fragment, "Checkout").addToBackStack(getString(R.string.actionCart)).commit();
            } else {
